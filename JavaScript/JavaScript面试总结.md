...
如果有一个操作数是布尔值，则会先隐式调用 `Number()` 转型函数，将其转换为数值，再进行判断。

如果有一个操作数是字符串，另一个是数值，则隐式调用 `Number()` 转型函数，将字符串转换为数值，再进行判断。

如果有一个操作数是对象，则调用对象的 `valueOf()` 方法，如果返回的是复杂数据类型，则在返回值得基础上再调用 `toString()` 方法，最后根据以上规则进行比较。
